...
public abstract class CodedOutputStream extends ByteOutput {
  ...
  public static CodedOutputStream newInstance(ByteBuffer buffer) {
    ...
    if (buffer.hasArray()) {
      ...
      return new OutputStreamEncoder(
          new ByteBufferOutputStream(buffer), Math.min(buffer.remaining(), DEFAULT_BUFFER_SIZE));
    } else {
      throw new IllegalArgumentException("ByteBuffer must be either direct or array");
    }
  }
  ...
  private static final class ArrayEncoder extends CodedOutputStream {
    ...
    /**
     * A flusher so that when we are writing out to a ByteBuffer which is backed by an array we can
     * flush to set the ByteBuffer's position. When we're writing out to a flat array this is null.
     */
  ...
  private static final class ByteBufferOutputStream extends OutputStream {
    ...
    public void write(int b) throws IOException {
      try {
        ...
        ioobe.initCause(e);
        throw new OutOfSpaceException(byteBuffer.position(), byteBuffer.limit(), 1, ioobe);
      }
    }
    ...
    public void write(byte[] b, int off, int len) throws IOException {
      try {
        ...
        IndexOutOfBoundsException ioobe = new IndexOutOfBoundsException(e.getMessage());
        ioobe.initCause(e);
        throw new OutOfSpaceException(byteBuffer.position(), byteBuffer.limit(), len, ioobe);
      } catch (IndexOutOfBoundsException e) {
        throw new OutOfSpaceException(byteBuffer.position(), byteBuffer.limit(), len, e);
      }
    }
  }
  ...
  private static final class OutputStreamEncoder extends CodedOutputStream {
    ...
    public final int spaceLeft() {
      if (out instanceof ByteBufferOutputStream) {
        return Math.max(0, ((ByteBufferOutputStream) out).spaceLeft() - position);
      }
